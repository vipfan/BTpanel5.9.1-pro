...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh https://raw.githubusercontent.com/vipfan/BTpanel5.9.1-pro/master/install-ubuntu.sh && bash install.sh
		exit;
	else
...
"
get_node_url(){
	nodes=(https://raw.githubusercontent.com/vipfan/BTpanel5.9.1-pro/master);
	i=1;
	if [ ! -f /bin/curl ];then
	...
	if [ $j = 5000 ];then
		NODE_URL='https://raw.githubusercontent.com/vipfan/BTpanel5.9.1-pro/master';
	else
		NODE_URL=${urls[$j]}
	fi
